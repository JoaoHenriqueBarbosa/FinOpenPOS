...
import {
	incrementNextNumber,
	loadFiscalSettings,
} from "./fiscal-settings-repository";
import {
	findInvoice,
	findPendingInvoices,
	loadOrderWithItems,
...
/** FiscalSettings com certificado garantidamente configurado. */
type SettingsWithCertificate = FiscalSettings & {
...
async function loadValidatedSettings(
	userUid: string,
): Promise<SettingsWithCertificate> {
	...
	return settings as SettingsWithCertificate;
}
...
export async function issueInvoice(
	...
	const number =
		model === 65 ? settings.nextNfceNumber : settings.nextNfeNumber;
...
export async function checkSefazStatus(userUid: string): Promise<{
	...
	const requestXml = buildStatusRequestXml(
		settings.stateCode,
		settings.environment,
	);
...
export async function voidNumberRange(
	...
	const { parseStatusResponse } = await import(
		"@finopenpos/fiscal/sefaz-client"
	);
