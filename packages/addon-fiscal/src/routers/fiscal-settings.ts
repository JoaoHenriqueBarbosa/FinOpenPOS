...
import { checkSefazStatus } from "../invoice-service";
import { fiscalSettings } from "../schema";
...
export const fiscalSettingsRouter = router({
	...
	upsert: protectedProcedure
		.meta({
			openapi: {
				...
				path: "/fiscal-settings",
				tags: ["Fiscal"],
				summary: "Create or update fiscal settings",
			},
		})
		.input(
			z.object({
	...
	testConnection: protectedProcedure
		.meta({
			openapi: {
				method: "POST",
				path: "/fiscal-settings/test-connection",
				tags: ["Fiscal"],
				summary: "Test SEFAZ connection",
			},
		})
		.input(z.void())
		.output(
	...
	certificateInfo: protectedProcedure
		.meta({
			openapi: {
				method: "GET",
				path: "/fiscal-settings/certificate",
				tags: ["Fiscal"],
