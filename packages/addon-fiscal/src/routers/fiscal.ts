import { protectedProcedure, router } from "@finopenpos/api";
...
export const fiscalRouter = router({
		...
		.query(async ({ ctx, input }) => {
			const invoice = await db.query.invoices.findFirst({
				where: and(
					...
					eq(invoices.user_uid, ctx.user.id),
				),
				with: {
					items: true,
					events: {
	...
	emit: protectedProcedure
		.meta({
			openapi: {
				method: "POST",
				path: "/fiscal/invoices/emit",
				tags: ["Fiscal"],
				summary: "Emit an invoice",
			},
		})
		.input(
			z.object({
	...
	cancel: protectedProcedure
		.meta({
			openapi: {
				method: "POST",
				path: "/fiscal/invoices/{id}/cancel",
				tags: ["Fiscal"],
	...
	voidRange: protectedProcedure
		.meta({
			openapi: {
				method: "POST",
				path: "/fiscal/invoices/void-range",
				tags: ["Fiscal"],
				summary: "Void a number range",
			},
		})
		.input(
			z.object({
	...
	syncPending: protectedProcedure
		.meta({
			openapi: {
				method: "POST",
				path: "/fiscal/invoices/sync",
				tags: ["Fiscal"],
				summary: "Sync pending contingency invoices",
			},
		})
		.input(z.void())
		.output(
	...
	downloadXml: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			const invoice = await db
				...
				.from(invoices)
				.where(
					and(eq(invoices.id, input.id), eq(invoices.user_uid, ctx.user.id)),
				)
				.limit(1);
