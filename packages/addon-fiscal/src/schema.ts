/**
 * Tabelas próprias do addon fiscal (contribution point de schema).
 *
 * O domínio fiscal permanece tabular por ser espelho de registro externo
 * imutável (XML assinado + protocolo SEFAZ) — ver docs/12-event-sourcing.md.
 * `order_id`/`product_id` referenciam streams do event store do host
 * (sem FK: os agregados operacionais vivem no log `events`).
 */
...
/** Tabelas em ordem FK-safe (contribution point de schema). */
export const fiscalTables = [
	cities,
	fiscalSettings,
	invoices,
	invoiceItems,
