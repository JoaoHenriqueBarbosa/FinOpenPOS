...
import { useAddonServices } from "@finopenpos/addon-kit/ui";
...
export default function FiscalPage() {
	...
	const syncMutation = useMutation(
		trpc.fiscal.syncPending.mutationOptions({
			onSuccess: (data) => {
				toast.success(
					t("syncResult", { authorized: data.authorized, total: data.total }),
				);
				queryClient.invalidateQueries(trpc.fiscal.list.queryOptions());
			},
		}),
	...
	const columns: Column<Invoice>[] = [
		...
		},
		{
			...
			hideOnMobile: true,
			accessorFn: (row) =>
				row.issued_at ? new Date(row.issued_at).getTime() : 0,
			render: (row) =>
	...
	const filteredInvoices = (invoices as Invoice[]).filter((inv) => {
		...
		if (modelFilter !== "all" && String(inv.model) !== modelFilter)
			return false;
		if (searchTerm) {
	...
	const hasPending = (invoices as Invoice[]).some(
		(i) => i.status === "contingency",
	);
			...
			<Card className="flex flex-col gap-4 p-3 sm:gap-6 sm:p-6">
				...
				<CardContent className="p-0">
					<DataTable
						...
						emptyIcon={<ReceiptTextIcon className="h-8 w-8" />}
