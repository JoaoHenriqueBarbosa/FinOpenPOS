...
export default function FiscalSettingsPage() {
	...
	const { data: settings, isLoading } = useQuery(
		trpc.fiscalSettings.get.queryOptions(),
	) as { data: any; isLoading: boolean };
	const { data: certInfo } = useQuery(
		trpc.fiscalSettings.certificateInfo.queryOptions(),
	) as { data: any };
	...
	const upsertMutation = useMutation(
		trpc.fiscalSettings.upsert.mutationOptions({
			onSuccess: () => {
	...
	const testMutation = useMutation(
		trpc.fiscalSettings.testConnection.mutationOptions({
			onSuccess: (data) => {
				if (data.online) {
	...
	const form = useForm({
		...
		onSubmit: ({ value }) => {
			upsertMutation.mutate({
				...
				tax_regime: Number.parseInt(value.tax_regime),
				...
				environment: Number.parseInt(value.environment),
				...
				<CardContent className="grid gap-4">
					<div className="grid gap-4 sm:grid-cols-3">
							...
							{(field) => (
								<div className="space-y-2">
									...
									<div className="relative">
										...
										/>
										{cepLoading && (
											<Loader2Icon className="absolute top-2.5 right-2 h-4 w-4 animate-spin text-muted-foreground" />
										)}
									</div>
								</div>
							)}
						...
						<form.Field name="state_code">
							{(field) => {
								const stateItem = STATE_OPTIONS.find(
				...
				<CardContent className="grid gap-4">
					{certInfo && (
						<div className="space-y-2 rounded-lg border p-4">
							...
							<div className="text-muted-foreground text-sm">
								<p>
									{t("certificateCommonName")}: {certInfo.commonName}
								</p>
								<p>
									{t("certificateExpiry")}:{" "}
									{new Date(certInfo.validUntil).toLocaleDateString()}
								</p>
							</div>
						</div>
					)}
					...
					<div className="space-y-2">
						...
						<div className="flex items-center gap-2">
							...
							/>
							{certFileName && (
								<Badge variant="secondary">{certFileName}</Badge>
							)}
						</div>
					</div>
