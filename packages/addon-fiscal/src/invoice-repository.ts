...
export async function updateInvoiceStatus(
	...
	if (data.responseXml !== undefined)
		updateFields.response_xml = data.responseXml;
	if (data.protocolXml !== undefined)
		updateFields.protocol_xml = data.protocolXml;
	if (data.protocolNumber !== undefined)
		updateFields.protocol_number = data.protocolNumber;
	if (data.statusCode !== undefined) updateFields.status_code = data.statusCode;
	if (data.statusMessage !== undefined)
		updateFields.status_message = data.statusMessage;
	if (data.authorizedAt !== undefined)
		updateFields.authorized_at = data.authorizedAt;

	await db.update(invoices).set(updateFields).where(eq(invoices.id, invoiceId));
}
