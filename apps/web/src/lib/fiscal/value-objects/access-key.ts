...
export class AccessKey {
  ...
  /** IBGE state code — positions 0..1 */
  stateCode(): string {
    return this.value.substring(0, 2);
  }

  /** Year + month (AAMM) — positions 2..5 */
  yearMonth(): string {
    return this.value.substring(2, 6);
  }

  /** Taxpayer CNPJ — positions 6..19 */
  taxId(): string {
    return this.value.substring(6, 20);
  }

  /** Fiscal model (55 = NF-e, 65 = NFC-e) — positions 20..21 */
  model(): number {
    return parseInt(this.value.substring(20, 22));
  }

  /** Series number — positions 22..24 */
  series(): number {
    return parseInt(this.value.substring(22, 25));
  }

  /** Invoice number — positions 25..33 */
  number(): number {
    return parseInt(this.value.substring(25, 34));
  }

  /** Emission type — position 34 */
  emissionType(): number {
    return parseInt(this.value.substring(34, 35));
  }

  /** Numeric code — positions 35..42 */
  numericCode(): string {
    return this.value.substring(35, 43);
  }

  /** Check digit (mod-11) — position 43 */
  checkDigit(): string {
    return this.value.substring(43, 44);
  }

  /** Returns the raw 44-digit string. */
  toString(): string {
    return this.value;
  }
}
